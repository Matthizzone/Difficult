...
public class AudioManager : MonoBehaviour
{
    ...
    public void SetAll(float volume, float pitch, SoundType type, bool loop, float pitch_randomize, bool start_randomize, int round_robin, bool crowd_control, float new_3D)
    {
        Volume = volume;
        Pitch = pitch;
        Type = type;
    ...
    public AudioSource PlaySound(string clip_name, bool reset)
    {
        ...
        if (ThreeD == 1) AS.maxDistance = 50;
        if (ThreeD == 1) AS.dopplerLevel = 0.5f;
        AS.Play();
